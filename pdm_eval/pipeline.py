...
from .metrics.alarm_islands import (
...
def _extract_theta_sweep_event_metrics(event_results: dict) -> dict:
    """Flatten full event metrics for one theta row."""
    ...
    def nab_value(profile: str, key: str):
        return _optional_float(nab.get(profile, {}).get(key))

    def nab_count(profile: str, key: str):
        return _optional_int(nab.get(profile, {}).get(key))

    return {
        "far_per_week": _optional_float(far.get("far_per_week")),
        "faa": _optional_float(faa.get("first_alarm_accuracy")),
        "nab_standard": nab_value("standard", "nab_score_normalized"),
        "ttd_mean_min": _optional_float(ttd.get("mean_ttd")),
        "ttd_median_min": _optional_float(ttd.get("median_ttd")),
        "ttd_std_min": _optional_float(ttd.get("std_ttd")),
        "ttd_min_min": _optional_float(ttd.get("min_ttd")),
        "ttd_max_min": _optional_float(ttd.get("max_ttd")),
        "ttd_detected_events": _optional_int(ttd.get("detected_events")),
        "ttd_missed_events": _optional_int(ttd.get("missed_events")),
        "faa_tp_events": _optional_int(faa.get("tp_events")),
        "faa_first_alarm_in_window": _optional_int(faa.get("first_alarm_in_window")),
        "coverage_percent": _optional_float(coverage.get("alarm_coverage_percent")),
        "alarm_points": _optional_int(coverage.get("alarm_points")),
        "total_points": _optional_int(coverage.get("total_points")),
        "mtia_mean_min": _optional_float(mtia.get("mtia_minutes")),
        "mtia_median_min": _optional_float(mtia.get("median_minutes")),
        "mtia_std_min": _optional_float(mtia.get("std_minutes")),
        "mtia_min_min": _optional_float(mtia.get("min_minutes")),
        "mtia_max_min": _optional_float(mtia.get("max_minutes")),
        "mtia_num_intervals": _optional_int(mtia.get("num_intervals")),
        "far_per_day": _optional_float(far.get("far_per_day")),
        "far_fp_intervals": _optional_int(far.get("fp_intervals")),
        "far_total_alarm_intervals": _optional_int(far.get("total_alarm_intervals")),
        "nab_standard_raw": nab_value("standard", "nab_score_raw"),
        "nab_standard_num_fp": nab_count("standard", "num_fp"),
        "nab_low_fp": nab_value("low_fp", "nab_score_normalized"),
        "nab_low_fp_raw": nab_value("low_fp", "nab_score_raw"),
        "nab_low_fp_num_fp": nab_count("low_fp", "num_fp"),
        "nab_low_fn": nab_value("low_fn", "nab_score_normalized"),
        "nab_low_fn_raw": nab_value("low_fn", "nab_score_raw"),
        "nab_low_fn_num_fp": nab_count("low_fn", "num_fp"),
    }


def _build_theta_sweep_pr_rows(
    theta: float,
    event_results: dict,
    selection_mode: str,
) -> List[dict]:
    pr = event_results.get("pr_leadtime", {})
    lead_times = pr.get("lead_times", []) or []
    if not lead_times:
        return []
    ...
    n = min([len(lead_times)] + [len(values) for values in columns.values()])
    rows: List[dict] = []
    for i in range(n):
        rows.append(
            {
                "maintenance_risk_theta": theta,
                "lead_time_min": int(lead_times[i]),
                "precision": _optional_float(columns["precision"][i]),
                "recall": _optional_float(columns["recall"][i]),
                "f1": _optional_float(columns["f1"][i]),
                "tp": _optional_int(columns["tp"][i]),
                "fp": _optional_int(columns["fp"][i]),
                "fn": _optional_int(columns["fn"][i]),
...
def _build_theta_sweep_ttd_distribution_rows(
    theta: float,
    event_results: dict,
    selection_mode: str,
) -> List[dict]:
    ...
    n = min(len(counts), max(0, len(bins) - 1))
    rows: List[dict] = []
    for i in range(n):
        rows.append(
            {
                "maintenance_risk_theta": theta,
                "bin_start_min": _optional_float(bins[i]),
                "bin_end_min": _optional_float(bins[i + 1]),
                "count": _optional_int(counts[i]),
                "selection_mode": selection_mode,
            }
        )
    return rows
...
def _save_theta_sweep_companion_csv(
    filename: str,
    rows: List[dict],
    *,
    mode: str,
    ...
    if not rows:
        return
    out_path = _pred_output_path(filename, mode, detector)
    if not out_path:
        return
    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows).to_csv(out_path, index=False)
    print(f"[INFO] Saved maintenance_risk_theta companion sweep: {out_path}")


THETA_SWEEP_NON_SUMMARY_COLUMNS = {"maintenance_risk_theta", "selection_mode"}
...
def _build_theta_sweep_summary(rows: List[dict]) -> pd.DataFrame:
    ...
    for metric in df.columns:
        if metric in THETA_SWEEP_NON_SUMMARY_COLUMNS:
            continue
...
def _save_maintenance_risk_theta_sweep(
    ...
    *,
    ...
    rows = []
    pr_rows: List[dict] = []
    ttd_distribution_rows: List[dict] = []
    for row in risk_results:
        ...
        for metric, value in _extract_theta_sweep_event_metrics(event_results).items():
            out_row[metric] = value
        rows.append(out_row)
        pr_rows.extend(_build_theta_sweep_pr_rows(theta, event_results, selection_mode))
        ttd_distribution_rows.extend(
            _build_theta_sweep_ttd_distribution_rows(theta, event_results, selection_mode)
        )

    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows).to_csv(out_path, index=False)
    print(f"[INFO] Saved maintenance_risk_theta sweep: {out_path}")
    _save_theta_sweep_companion_csv(
        "theta_sweep_pr_vs_lead_time.csv",
        pr_rows,
        mode=mode,
        detector=detector,
    )
    _save_theta_sweep_companion_csv(
        "theta_sweep_ttd_distribution.csv",
        ttd_distribution_rows,
        mode=mode,
        detector=detector,
    )
